...
export default function MonthView({ calendarData, currentDate, onSessionClick, onDayClick, density = 'comfortable' }: MonthViewProps) {
    ...
    <div className="h-full flex flex-col bg-white rounded-xl overflow-hidden shadow-lg border border-gray-200">
      ...
      <div className="flex-1 overflow-auto">{
        ...
        <div 
          ...
          style={{ 
            gridTemplateColumns: `repeat(${Math.min(7, calendarGrid.filter(Boolean).length)}, minmax(120px, 1fr))` 
          }}
        >
          {calendarGrid.map((gridDay, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Holiday indicator */}
                {dayData.is_holiday && dayData.holiday_info && isCurrentMonth && (
                  <div className="text-[10px] text-red-600 mb-1 truncate font-medium">
                    🎌 {(dayData.holiday_info as { name?: string })?.name || 'Holiday'}
                  </div>
                )}
                ...
                {isCurrentMonth && dayData.sessions.length > 0 && (
                  <div className="space-y-1 overflow-hidden">
                    {Object.entries(sessionsByBranch).slice(0, 3).map(([branchName, branchSessions]) => (
                      <div key={branchName} className="space-y-0.5">
                        {branchSessions.slice(0, 2).map((session) => (
                          <div
                            key={session.id}
                            className={`text-[10px] p-1 rounded border cursor-pointer hover:shadow-sm transition-all ${getBranchColorLight(branchName)}`}
                            onClick={(e) => {
                              e.stopPropagation();
                          ...
                          >
                            <div className="flex items-center justify-between mb-0.5">
                              ...
                              <div className={`w-2 h-2 rounded-full ml-1 ${getBranchColorDot(branchName)}`}></div>
                            </div>
                ...
                {isCurrentMonth && dayData.session_count > 0 && (
                  <div className="absolute top-1 left-1">
                    <div className={`w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center text-white ${
                      dayData.session_count > 5 ? 'bg-red-500' :
                      dayData.session_count > 3 ? 'bg-orange-500' :
                      'bg-green-500'
                    }`}>
                ...
                {isCurrentMonth && dayData.sessions.length === 0 && !dayData.is_holiday && (
                  <div className="h-full flex items-center justify-center opacity-0 hover:opacity-60 transition-opacity">
                    <div className="w-8 h-8 rounded-full bg-indigo-100 hover:bg-indigo-200 flex items-center justify-center">
                      <span className="text-lg text-indigo-400 hover:text-indigo-600">+</span>
                    </div>
                  </div>
                )}
              </div>
