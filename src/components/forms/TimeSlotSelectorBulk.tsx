              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <Select
                      value={slot.day}
                      onChange={(event: React.ChangeEvent<HTMLSelectElement>) => updateTimeSlot(slot.id, 'day', event.target.value)}
                      options={daysOfWeek}
                      placeholder={language === 'th' ? 'เลือกวัน' : 'Select Day'}
                      disabled={false}
                    />
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <Select
                      value={slot.timeFrom}
                      onChange={(event: React.ChangeEvent<HTMLSelectElement>) => updateTimeSlot(slot.id, 'timeFrom', event.target.value)}
                      options={timeOptions}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <Select
                      value={slot.timeTo}
                      onChange={(event: React.ChangeEvent<HTMLSelectElement>) => updateTimeSlot(slot.id, 'timeTo', event.target.value)}
                      options={timeOptions.filter(time => !slot.timeFrom || time.value > slot.timeFrom)}
                      placeholder={language === 'th' ? 'เลือกเวลา' : 'Select Time'}
                      disabled={false}
                    />
                </div>
